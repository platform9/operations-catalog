...
def test_query_metrics_maps_values_to_status():
    with patch("health_store.requests.get", side_effect=_mock_get(METRICS_RESPONSE, LOGS_RESPONSE)):
        import health_store
...
def test_query_logs_returns_latest_detail_per_check():
    with patch("health_store.requests.get", side_effect=_mock_get(METRICS_RESPONSE, LOGS_RESPONSE)):
        import health_store
        result = health_store._query_logs("bork")
...
def test_get_service_health_overall_fail_takes_priority():
    ...
    with patch("health_store.requests.get", side_effect=_mock_get(fail_metrics, empty_logs)):
        import health_store
        result = health_store.get_service_health("bork")
...
def test_get_service_health_no_checks_returns_pass():
    empty = {"data": {"result": []}}
    with patch("health_store.requests.get", side_effect=_mock_get(empty, empty)):
        import health_store
        result = health_store.get_service_health("bork")
